...
表格与结构提取必须调用 PP-Structure，并优先利用 PP-Structure 输出的 HTML 结构识别 `rowspan`、`colspan`、跨行单元格、跨列单元格、表格行列结构、单元格文本和单元格位置。复杂表格不得仅依赖 OCR 文本顺序重建，也不得忽略合并单元格结构，更不得用大语言模型猜测缺失单元格内容。复杂表格应先解析 PP-Structure HTML，再构建 DataFrame，随后展开合并单元格并完成字段标准化。
