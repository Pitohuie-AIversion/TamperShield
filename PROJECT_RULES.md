...
在旧流程完全废弃前，禁止删除或破坏 `main.py::run_tamper_shield_pipeline(...)`。

如果需要让 `main.py` 支持 Document-first pipeline，只能新增并行入口，例如：

```text
run_document_first_pipeline(...)
```

不得直接把旧 table-first pipeline 改写成新流程，也不得破坏现有参数、返回值和写入权限控制。
