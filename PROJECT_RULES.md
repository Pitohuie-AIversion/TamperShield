...
后续 Phase 9 的报告导出必须以 `EvidenceIndex` 为输入，不得在报告模块中重新执行文档解析、页面对齐、内容比对或表格精查。
...
```text
Legacy table-first pipeline
  - 保留用于兼容已有表格 OCR / DataFrame 比对流程
  - 不作为新的主架构方向

Document-first pipeline
  - 位于 core/document_pipeline.py
  - 作为新的工程文档防篡改核验主线
  - 返回 EvidenceIndex
  - 默认不写文件、不生成报告
```
...
DOCX 和 PDF 同时存在时，应先进入文档级对齐：DOCX 提供可编辑的基准内容顺序，PDF 提供页码、版式、坐标和发布态追溯校验。不得默认只把 DOCX 表格作为 `base_df` 并跳过逐页内容核验。
...
不得自动生成最终保存路径或文件名。Phase 9 报告导出必须显式传入输出路径，且写文件必须要求 `allow_write=True`；不得覆盖已有文件，除非 `overwrite=True`。
