...
当前重点：基于 Phase 11e 的真实验证结果，进入 targeted issue analysis，而不是大规模重构。
...
### Phase 11f / 后续 targeted issues

- [ ] 分析 PDF 39 页 vs DOCX rendered 45 页的页面对齐差异
- [ ] 检查 `page_reordered=3` 是否由页码错配、目录页、封面页或空白页导致
- [ ] 检查 `page_deleted=8` 与 `page_added=2` 的具体页码
- [ ] 检查为什么 rendered PDF 路径下 `requires_table_compare_count=0`
- [ ] 评估是否需要在 rendered PDF 页面中识别 table-like blocks，或保留 DOCX native table metadata 作为辅助元素
- [ ] 不要在没有真实差异定位证据前重构核心架构
